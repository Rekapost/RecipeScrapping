...
//	int p;
	...
	public String[][] getExcelData() throws IOException {
		String path = "C:\\Users\\Reka\\eclipse-workspace\\webscrapping\\src\\test\\resources\\ScrapeData\\Hypothyroid_Input.xlsx";
		reader = new ExcelReadWrite(path);
		noOfRows = reader.getRowCount("hypothyroid_list");
		int noOfColumns = reader.getCellCount("hypothyroid_list", 1); // 1 is rownumber
		...
		for (int m = 1; m <= noOfRows; m++) {
			...
			for (int j = 0; j < noOfColumns; j++) {
				// j=0 ,< ; j=1 ,<=
				dataTable[m - 1][j] = reader.getCellData("hypothyroid_list", m, j); // 0,0= 1,0
			}
		}
		return dataTable;
	}
...
//		int pageCounter = 2;
		...
		String path = "C:\\Users\\Reka\\eclipse-workspace\\webscrapping\\src\\test\\resources\\ScrapeData\\Hypothyroid_Input.xlsx";
		reader = new ExcelReadWrite(path);
		int rowSize = reader.getRowCount("hypothyroid_list");
...
// reading diabetes eliminated list		
		...
		int hypoRowSize = reader.getRowCount("hypothyroid_eliminated");
		System.out.println(" row size " + hypoRowSize);
		System.out.println("Reading excel ");

		for (int d = 1; d <= hypoRowSize; d++) {
			String hypothyroid_data = reader.getCellData("hypothyroid_eliminated", d, 0);
...
//				Thread.sleep(1000);
				...
				boolean isContainEliminatedItems = common.hasEliminatedLists(hypothyroid_eli_List, RecpieIngredients);
...
//				// Thread.sleep(1000);
					...
					if (searchinput.equalsIgnoreCase("Hypothyroid Breakfast")) {
						try {
							common.writeDataToExcel(scrapedData, "breakfastHypothyroidRecipes", searchinput);
						} catch (IOException e) {
